...
> **v3.94: 정렬 구현은 `backend/top3Ranking.js` 단일 출처.** 세 곳에 흩어져 있었고
> `weekly-diagnostic.js`는 `total_score` 내림차순이라는 제3의 기준을 쓰고 있었다.
> 스윗스팟은 **비단조**(50-59 > 90+)라 점수 정렬과 순서가 뒤집힌다 — 실측 57%의 날에
> 진단의 TOP1이 실제 🥇와 달랐다. **"TOP1"을 다루는 분석 코드는 반드시
> `resolveTop3Order()`를 쓸 것** (저장된 `top3_rank` 우선, 없으면 v387 재구성).
> `total_score`로 정렬하면 안 된다.
1. **수급등급(sg)**: 외인≥2d 단독(5) > 쌍방≥2d(4) > 기관≥2d(3) > 외인≥1d(2) > 고래만(1)
2. **기관매수일**: 내림차순
3. **스윗스팟 구간**: 50-59점(1) > 60-69점(2) > 80-89점(3) > 90+점(4) > 70-79점(5) > 45-49점(6)

> **⚠️ v3.94: 이 근거는 오염된 입력 위에서 산출됐다.** 504개 조합 탐색이 쓴
> `institution_buy_days`/`foreign_buy_days`가 방향 버그(가장 오래된 날부터 스트릭을 셈)로
> 계산된 값이었다. 재검증(`scripts/revalidate-supply-sort.js`, 2026-05-22~07-16, 28일/78건)에서
> **올바른 수급 기준 v387의 TOP1 알파는 +0.02%p로 사실상 0** (오염값 기준으로는 +1.58%p로
> 좋아 보였다). 단 n=23일·KOSPI −13.4% 단일 폭락 구간이라 **기각도 확정도 불가**.
> 올바른 수급 이력이 5월 하순부터만 존재하므로 **전수 재탐색은 데이터 축적 후**에 가능하다.
> 그때까지 v3.87은 "관성으로 유지 중이며 근거는 미검증" 상태로 취급할 것.
...
**시총 플로어 + 레짐 조건부** (v3.90~3.92): 정렬 직전 `applyMomentumCapFloor()` 적용.
- `detectMarketRegime()`: 직전 10거래일 (KOSPI−KOSDAQ) 누적 스프레드 <0 **AND** KOSPI 10일 누적 >0 → 'broad'(플로어 OFF), 그 외 'momentum'(플로어 ON). 하락장의 spread<0은 위험회피이지 소형주 랠리가 아님(v3.92).
- momentum 레짐: 5조+ 우선 → 3개 미달 시 1조+ 폴백 → **그래도 없으면 무픽(빈 TOP3)**. 소형주 원본 폴백은 v3.92에서 제거(폴백 픽 나노캠텍 -19.1% vs 자연 무픽일 6/23이 -8.9% 폭락일).
- 레짐은 save 모드에서 `market_regime` 컬럼으로 저장, alert 모드는 DB값 사용.
...
### 4. TOP1/2/3 정렬 로직 재최적화 (대기: 2026-06-15 이후)

- **내용**: v387 정렬(수급등급→기관매수일→스윗스팟)이 실제 성과와 불일치 가능성 확인
- **근거**: 2026-05-25 분석(n=13)에서 TOP3 성과가 TOP1보다 D+1/D+5/D+10 모두 우월. TOP2가 전 구간 최하위.
- **리스크**: 표본 13건으로 통계적 신뢰도 부족. 역선택일 수도 있고 우연일 수도 있음.
- **검증 방법**: `scripts/sort-optimizer.js` 재실행 → 순위별(TOP1/2/3) D+1, D+3, D+10 성과를 504개 정렬 조합으로 전수 탐색. TOP1 D+10 승률 60%+ 조합이 현 v387보다 우위일 때 교체.
- **검증 시점**: D+10 데이터 30건+ 확보 예상 시점 (2026-06-15 이후)

### 5. score_health broken 시 수급 중심 필터 전환 (대기: 검증 필요)

- **내용**: 주간 진단 `score_health_label = 'broken'` 상태에서 TOP3 선별 기준을 점수 중심 → 순수 수급(기관≥2일 OR 외인≥2일 OR 확인된 고래) 중심으로 전환
- **근거**: 2026-05-11, 05-18 2주 연속 broken 상태에서 점수 높은 종목(A~S등급)이 B등급보다 실제로 더 좋지 않은 사례 다수
- **리스크**: broken 상태가 일시적(2~3주)이라면 과적응. 수급 중심 전환 시 TOP3가 없는 날 발생 가능.
- **검증 방법**: broken 구간(05/11~) TOP3의 등급별 실제 D+3 수익을 정상 구간과 비교. broken 시 B등급 수익 > A등급 수익이 통계적으로 유의하면 적용.
- **검증 시점**: broken 구간 추천 종목 D+3 데이터 15건+ 확보 후 (2026-06-01 이후)

### 6-A. 깔때기 뒤집기 검증 — 풀 밖 수급-우선 신호 (v3.94 설계 개정)

- **내용**: `market_flow_daily`(전 종목 일별 수급+가격)에서 "순위에 뜨기 전" 매집 신호(기관/외인 연속 순매수 + 거래량 점증 + 가격 횡보)를 형성해, **풀 밖 수급 신호 vs 매칭 대조군 vs 현행 TOP3 vs KOSPI**를 비교.
- **배경(2026-07-06 구조 진단)**: 현행 풀(거래량 순위 top30)은 주목 정점 이후 표본이라 음의 드리프트(4~6월 풀 전체 -2.0%/승30%), 건당 알파 전 기간 -6~-7.5%p. 철학("조용한 매집 선점")과 표본추출("가장 시끄러운 30개")의 모순이 근본 원인 가설.

> #### ⚠️ v3.94 설계 개정 (2026-07-17) — 원안은 실행 불가였다
>
> **(1) 시점 근거가 틀렸다.** 원안은 "백필(5월 말~) + 신규 2주면 확보"였으나, 실측 결과
> `market_flow_daily`의 전 종목 수집 구간은 **2026-05-22~07-16의 38거래일뿐**이다.
> 신호일 수 = 전체 − L(신호형성 깊이) − H(평가지평):
>
> | 설계 | 신호일 | 신호종목 | 일평균 | **독립블록** |
> |---|---|---|---|---|
> | 원안 (L=20, streak=5) | 8 | 146 | 18.3 | **0.8** |
> | 완화 (L=10, streak=3) | 18 | 1,298 | 72.1 | **1.8** |
> | 최대완화 (L=5, streak=2) | 23 | 10,354 | 450.2 | **2.3** |
>
> **핵심: 필터를 완화해 표본을 70배 늘려도 독립블록은 0.8→2.3에 그친다.** 제약은 필터가
> 아니라 달력이다. D+10 윈도우가 겹치므로 겹친 코호트는 **같은 10일 시장 구간을 반복 측정**할
> 뿐이다. 블록<2면 "폭락장 10일 1회"를 본 것이고, 어떤 수치가 나와도 결론이 아니다.
> (독립블록 = 신호일 ÷ H)
>
> **(2) 절대수익 비교는 베타에 잡아먹힌다.** 원안의 "vs KOSPI"만으로는 부족하다. 신호가
> 소형주에 쏠리면 폭락장에선 신호 탓이 아니라 사이즈 탓으로 나빠 보이고, 소형주 랠리엔
> 반대가 된다. v3.90~3.92 시총 플로어 saga가 이미 밟은 함정이다.
>
> **(3) 비교 단위가 불공정하다.** "신호 종목 전체 평균(일 72~450개)" vs "TOP3(3개)"는
> 광범위 스크린과 상위 3개 선별의 비교다. TOP3와 견주려면 신호 집합도 **랭킹해서 상위 3개**로 좁혀야 한다.
>
> #### 개정 설계
>
> - **주 지표 = 동일일·동일 시총분위 매칭 초과수익.** 같은 날 같은 시총 5분위의 비신호 종목
>   평균 대비 초과. 시장·사이즈 요인을 제거해야 "수급 신호 자체"의 알파가 보인다.
>   보조로 vs KOSPI(`overnight_predictions.kospi_close` 시계열 — `kospi_close_change` 신뢰 금지), vs 현행 TOP3.
> - **신호 형성 깊이 L=10** 주력(신호일 18일), L=20은 강건성 확인용.
> - **평가 지평 H=D+10 유지**(active_policy 지평, D+3 평가 금지 원칙). D+1/D+3/D+5 병기해 지평 민감도만 관측.
> - **TOP3 대응 비교**: 신호 집합을 수급강도 → 거래량 점증률 순으로 랭킹해 **일별 상위 3개**를 뽑아 현행 TOP3와 동수 비교.
> - **판정 시점을 분리한다.** (`blocks = (N − L − H) / H` → 블록 b 확보에 필요한 거래일 `N = L + H×(b+1)`)
>
>   | 독립블록 | L=10 필요N / 부족 | 달성 | L=20 필요N / 부족 | 달성 |
>   |---|---|---|---|---|
>   | 2 | 40d / +2d | 2026-07-21 | 50d / +12d | 2026-08-04 |
>   | **3 (판정 최소)** | **50d / +12d** | **2026-08-04** | 60d / +22d | 2026-08-19 |
>   | 4 | 60d / +22d | 2026-08-19 | 70d / +32d | 2026-09-02 |
>   | 6 | 80d / +42d | 2026-09-16 | 90d / +52d | 2026-10-06 |
>
>   - **2026-07-20 = 중간 관측**. 방향성과 파이프라인 검증만. **기각·채택 모두 금지**(블록 1.8).
>   - **정식 판정 = 2026-08-04**(L=10, 블록 3). L=20 강건성 확인은 08-19.
>   - 스크립트가 블록 3 미만이면 자동으로 "중간 관측"으로 출력하고 판정을 거부한다.
...
- **내용**: 추천 후 D+1에 -10% 이하 종목 발생 시 트래킹 메시지에 🚨 즉시 손절 권고 추가
- **근거**: 04/27 디바이스 D+1 -46.2% 사례. 현재는 D+1 손절 경고가 -5% 기준이나 추천 종목에 한해 더 빠른 대응 필요.
- **리스크**: 일시적 낙폭 후 반등 사례가 많으면 오히려 손해. D+1 -10% 이후 실제 최종 손실 vs 반등 비율 확인 필요.
- **검증 방법**: 현재 데이터에서 D+1 -10% 이하 사례의 D+3, D+5 수익 분포 확인. D+3에서도 손실 지속 비율 70%+ 시 구현.
- **검증 시점**: 즉시 확인 가능 (표본 수 부족하더라도 방향성 확인)
