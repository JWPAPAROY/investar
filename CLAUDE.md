...
KIS OpenAPI에서 3가지 순위를 KOSPI + KOSDAQ 각각 조회하여 종목 풀을 구성한다.

```
거래량 증가율 순위 200개 × 2시장 = 400개  (핵심 VPD 신호)
거래량 순위       100개 × 2시장 = 200개  (대형주 안전판)
거래대금 순위      50개 × 2시장 = 100개  (메가캡 보완)
────────────────────────────────────
합계: 700개 → ETF/ETN 필터링(15개 키워드) → 중복 제거 → 최종 ~120-150개
```
...
### v3.45 (2026-02-25)
- **종목 풀 대폭 확대**: 거래량증가율 100→200, 거래량순위 50→100, 거래대금 30→50 (총 700개/조회)
- **스크리닝 소요 시간 표시**: 결과 메타데이터 바에 스크리닝 소요 시간(초) 표시
- **종목 분석 탭 전략 토글**: 모멘텀/방어 전환 버튼 추가
- **과열 필터 완화**: RSI 80→85, 이격도 115→120
- **고래 감지 임계값 완화**: 소형주 2.5→2.0배, 중형주 2.0→1.5배, 대형주 1.5→1.2배
- **등락률 상승 API 제거**: VPD 철학과 역상관, 거래량증가율에 집중
