...
**정렬** (v3.86, B1 전략): 스윗스팟 구간 → 수급 → 점수. (tier1 시총 필터 없음)
1. **스윗스팟 구간** (낮을수록 우선): 50-59점(1) > 60-69점(2) > 80-89점(3) > 90+점(4) > 70-79점(5) > 45-49점(6)
2. **수급 tiebreak**: 외인≥2d 단독(5) > 쌍방≥2d(4) > 기관≥2d(3) > 외인≥1d(2) > 고래만(1)
3. **점수** (최종 tiebreak): 내림차순

**근거** (v3.86, 2026-05-05): 동일 데이터(1950건) 역대전략 비교. tier1 발동 20일 분석에서 tier1이 도움된 날 3일, 해가 된 날 5일. B1(tier1없음) D+1→D+10 평균 +6.62% / 승률 58% vs v383(tier1있음) +6.13% / 53%. 시총 구간 분석에서도 ≤1조가 >1조 대비 우월성 없음(승률 56% vs 62%).
