...
- **버전**: 3.24 (신호 효과성 분석 기반 스코어링 개선 - 매수고래만, 0점 지표 제거, 윗꼬리 감점)
- **최종 업데이트**: 2026-02-04
...
│ ─────────────────────────────────────────────────────────── │
│ 총점 = Base(0-25) + Whale(0-30) + Momentum(0-30)            │
│        + Trend(0-15) + SignalAdj [v3.24]                     │
│                                                              │
...
```
총점 (0-100점) = Base(0-25) + Whale(0-30) + Momentum(0-30) + Trend(0-15) + SignalAdj

SignalAdj (v3.24 NEW):
  탈출 속도 감지 → +5점 (승률 100%, 수익 +23.58%)
  윗꼬리 과다 → -10점 (승률 66.7%, 수익 +0.83%)

advancedIndicators totalScore 변경 (v3.24):
  유지: 고래(max 25), 탈출 속도(max 30), 비대칭(max 10)
  제거: accumulation, drain, gradualAccumulation, smartMoney, bottomFormation, breakoutPrep

고래 감지 변경 (v3.24):
  매수고래(🐋)만 +30점 보너스 및 TOP 3 후보
  매도고래(🐳)는 가점 없음, TOP 3 제외

추천 등급 (v3.24 실제 코드 기준, 7-Tier System)
...
### 2️⃣ Whale Score (0-30점) — 매수고래만 (v3.24)

**매수고래(🐋)만** 가점. 매도고래(🐳)는 수익률 -2.56%로 가점 부적절 → 제외.

| 항목 | 점수 | 설명 |
|------|------|------|
| 매수고래 감지 | 0 또는 30 | 거래량 2.5배+ && 가격 3%+ && 양봉 시 +30점 |
...
### 5️⃣ Signal Adjustments (v3.24 NEW)
...
├─ 되돌림: 8% → -1점
└─ 연속상승: 2일 → +1점
  → cap: min(18, 25) = 18점... 실제 0~25 범위

[Whale Score] 30점
└─ 매수고래 감지 → +30점

[Momentum Score] 20점
├─ 거래량 가속도: accelerating → 15점
├─ 연속 상승일: 2일 → 4점
└─ 기관 진입 가속: 1점
...
```javascript
// screening.js 최종 점수 계산
totalScore = Base(0-25) + Whale(0-30) + Momentum(0-30) + Trend(0-15)

// v3.24 Signal Adjustments
...
**Version**: 3.24 (신호 효과성 분석 기반 스코어링 개선)
**Author**: Claude Code with @knwwhr

**v3.24: 매수고래만 가점, 0점 지표 6개 제거, 윗꼬리 -10점, 탈출속도 +5점, 균형 신호 제거**
**🔧 등급: ⚠️과열(RSI>80 AND 이격도>115) > S+(90+) > S(75-89) > A(60-74) > B(45-59) > C(30-44) > D(<30)**
**📊 공식: Base(0-25) + Whale(0-30) + Momentum(0-30) + Trend(0-15) + SignalAdj = Total(0-100)**
...
## 📋 현재 점수 체계 상태 (v3.24 기준, 2026-02-04)
...
| Whale Score | 0 or 30 | ✅ 정상 | v3.24: 매수고래만 가점 |
...
#### 단기: v3.24 모니터링
- 매수고래 필터 + 윗꼬리 감점 + 탈출속도 보너스의 실전 성과 추적
- Supabase 성과 데이터로 v3.24 점수 분포 검증
