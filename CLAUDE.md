...
#### 1️⃣ 자동 재시도 로직
- 실패 종목 최대 3회 자동 재시도
- 재시도 간격: 1초 (KIS API rate limit 대응)
- 45초 타임아웃 방지 (Vercel 60초 제한)
- 최종 실패 목록 로깅

#### 2️⃣ 작동 방식
```
1차 시도: 121개 종목 → 110개 성공, 11개 실패
2차 시도 (1초 후): 11개 재시도 → 8개 성공, 3개 실패
3차 시도 (1초 후): 3개 재시도 → 2개 성공, 1개 실패
최종: 120/121개 성공 (99.2%)
```

**수정 파일**: `api/recommendations/update-prices.js`

---

### v3.27 (2026-02-05) - ALERT 실시간 스크리닝 + 종가 기준 성과 + 수동 가격 업데이트

#### 1️⃣ ALERT 모드 실시간 스크리닝
- 기존: Supabase 저장 데이터 조회 (전날 SAVE 결과)
- 변경: 실시간 `screenAllStocks()` 호출
- 효과: SAVE/ALERT 추천 종목 일관성 보장

#### 2️⃣ 성과 계산 종가 기준 통일
- 기존: `getCurrentPrice()` (시간외가 포함 가능)
- 변경: `getDailyChart()` 일봉 종가 사용
- 효과: 시간외 단일가 변동 배제, 일관된 성과 측정

#### 3️⃣ 과거 성과 Supabase 가격 사용 (타임아웃 방지)
- ALERT에서 과거 3일 성과 조회 시 KIS API 대신 Supabase 사용
- 실시간 스크리닝(30-45초) + Supabase 조회로 60초 내 완료

#### 4️⃣ update-prices 개선
- 가격 조회 실패 시 이전 가격 유지 (0% 버그 방지)
- 실패 종목 로그 출력 (`⚠️ 가격 미조회 [종목코드 종목명]`)
- 수동 업데이트 파라미터 추가: `?stockCode=001440&price=31750`

#### 5️⃣ 텔레그램 메시지 구조 개선
**SAVE 모드 (16:10)**: 🌆 오늘의 결산
- 어제 추천 당일 성과 (종가 기준 수익률)
- 내일을 위한 TOP 3 (현재가, 손절가, 최근 주가)

**ALERT 모드 (08:30)**: 🌅 오늘의 매수 전략
- 오늘의 TOP 3 (실시간 스크리닝, 손절가)
- 과거 3일 추천 성과 (Supabase 저장 종가 기준)

**수정 파일**: `api/cron/save-daily-recommendations.js`, `api/recommendations/update-prices.js`
