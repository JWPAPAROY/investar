...
B등급: 45-59 (매수 고려, 선행 패턴 감지, Supabase 저장 ✅)
C등급: 30-44 (관망, 약한 신호, 저장 제외 ❌)
D등급: <30 (비추천, 선행 지표 미감지)
```
...
## 📋 현재 점수 체계 상태 (v3.18.1 기준, 2026-01-31)

### 컴포넌트별 작동 현황

| 컴포넌트 | 배점 | 현재 상태 | 평균 획득 | 비고 |
|----------|------|----------|----------|------|
| Base Score | 0-15 | ✅ 정상 | ~10 | 거래량비율+OBV+VWAP+비대칭+유동성 |
| Momentum (거래량 가속) | 0-18 | ✅ 정상 | ~10 | D-5 vs D-0 비교 |
| Momentum (VPD 개선) | 0-12 | ✅ 정상 | ~8 | Volume-Price Divergence 변화 |
| Momentum (선행 지표 강화) | 0-10 | ✅ 정상 | ~7 | 패턴 강화 추세 |
| Momentum (기관 진입 가속) | 0-5 | ✅ v3.18.1 수정 | ~2 | D-5 vs D-0 기관 매수일 비교 |
| Momentum (당일 급등 페널티) | -20~0 | ✅ 정상 | ~-3 | 이미 급등 종목 필터 |
| Trend (거래량 점진 증가) | 0-20 | ✅ 정상 | ~12 | 30일 가속 패턴 |
| Trend (변동성 수축) | 0-10 | ⚠️ 시장 의존 | ~1 | 현재 시장 확장기라 대부분 0점 |
| Trend (기관/외국인 매집) | 0-5 | ✅ v3.18.1 수정 | ~3.6 | 총 매수일 카운트 |
| Trend (VPD 강화 추세) | 0-5 | ✅ 정상 | ~2 | VPD 장기 개선 |
| Multi-Signal 보너스 | 0-6 | ⚠️ 구조적 한계 | 0 | 대부분 1개 API 등장 |
| **합계** | **0-90** | | **~47** | |
