...
**정렬** (v3.86, v3.76 복귀): tier1(시총≤1조) 우선 → 스윗스팟 구간 → 수급 → 점수.
1. **tier1**: 시총 ≤1조(10,000억) 종목 우선. 3개 미달 시 전체로 확대
2. **스윗스팟 구간** (낮을수록 우선): 50-59점(1) > 60-69점(2) > 80-89점(3) > 90+점(4) > 70-79점(5) > 45-49점(6)
3. **수급 tiebreak**: 외인≥2d 단독(5) > 쌍방≥2d(4) > 기관≥2d(3) > 외인≥1d(2) > 고래만(1)
4. **점수** (최종 tiebreak): 내림차순

**복귀 근거** (v3.86, 2026-05-05): 동일 데이터(1950건, 2025-11-03~2026-05-04) 역대전략 비교에서 v376이 D+1→D+10 평균 +11.48%로 2위(v384 +7.62%) 대비 50% 우위. v385(isV2Priority)가 sg1(고래단독) 종목을 61%나 TOP1으로 올리는 구조적 문제로 성과 최하위(+4.40%) 기록.
...
**핵심 철학**: "충분히 빠진 + 바닥 다지는 = 안전한 반등 종목" (과매도 반등 중심, 수급은 보너스)

```
DefenseTotal = Recovery(0-35) + SupplyBonus(0-10) + Stability(0-25) + Safety(0-20) + SignalAdj
```

### 1. Recovery Score (0-35점) — 과매도 반등 신호 (v3.78: 30→35점 확대)

- **RSI 과매도 (0-14점)**: RSI 25-34 → 14점(반등 확률 최고), 20-24 → 12, 35-44 → 10, <20 → 10, 45-49 → 5, ≥50 → 0
- **MFI 회복 (0-12점)**: MFI 20-29 → 12, 15-19 → 9, 30-39 → 8, <15 → 6, 40-49 → 4, else → 0
- **이격도 할인 (0-9점)**: 90-94 → 9(20일선 대비 5-10% 할인), 85-89 → 8, <85 → 7, 95-97 → 5, 98-99 → 2, ≥100 → 0

### 2. Supply Bonus (0-10점) — 기관/외국인 수급 가산 (v3.78: 25→10점, 자격→보너스)

- **기관 연속매수일 (0-4점)**: ≥4일 → 4, 3 → 3, 2 → 2, 1 → 1
- **외국인 연속매수일 (0-4점)**: ≥4일 → 4, 3 → 3, 2 → 2, 1 → 1
- **쌍방 수급 보너스 (0-2점)**: 양쪽≥2 → 2, 양쪽≥1 → 1
...
**자격**: 비폭락, 시총≥1000억 (v3.78: SmartMoney/과열 자격 필터 제거)

**시총 단계적 확대**: 시총 ≤1조 종목 우선 → 3개 미달 시 시총 무제한 확대

**방어점수 내림차순 top3** (v3.78: 점수 하한 제거, 3월 데이터 검증 — 모멘텀 하락일 8회 중 7회(87.5%) 선방, 승률 69%/평균+3.29% vs 모멘텀 40%/+0.92%)
