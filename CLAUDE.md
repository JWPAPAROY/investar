...
**정렬** (v3.84): 전체 풀에서 단순 점수 내림차순.
1. **총점 1차** (내림차순)
2. **수급 tiebreak**: 외인≥2d 단독(5) > 쌍방≥2d(4) > 기관≥2d(3) > 외인≥1d(2) > 고래만(1)
3. **업종 주도도 tiebreak** (v3.82): `getSectorLeadingScore > 0.5`인 종목 우선

상위 3개 `slice(0, 3)`. **시총 티어(v3.63) / 스윗스팟 구간 우선순위(v3.76)는 v3.84에서 모두 제거**.

**제거 근거** (v3.84, 2026-04-16): POST 기간(2026-03-26~04-15) 15일 백테스트에서 구간 우선순위 + tier1 우선의 조합이 낮은 점수 소형주를 금메달로 올려 KEC(58점) -28.1%, 로킷헬스케어(53점) -28.5% 등 참사 유발. 순수 점수 내림차순이 금메달 승률 36→57%, 합산 -1.35% → +2.68%, -5% 손실률 48→29%로 모든 변형 중 최고 안정성 기록.
