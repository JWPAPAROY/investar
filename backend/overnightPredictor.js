 ...
 * 04:55 KST cron에서 호출 — 야간장(18:00~05:00) 마감 직전 데이터 캡처
 *
 * 야간선물은 정규선물과 동일 계약을 야간 시간에 거래하므로,
 * getKospi200FuturesPrice / getKosdaq150FuturesPrice를 직접 호출.
 * 야간장 중에는 정규선물 코드(10100000, A01606 등)로 야간 시세가 반환됨.
 */
async function saveNightFutures() {
  ...
  const futuresGetters = [
    { ticker: 'KOSPI200F', name: '코스피200 선물', getter: () => kisApi.getKospi200FuturesPrice() },
    { ticker: 'KOSDAQ150F', name: '코스닥150 선물', getter: () => kisApi.getKosdaq150FuturesPrice() },
  ];

  const results = [];

  for (const { ticker, name, getter } of futuresGetters) {
    try {
      ...
      if (result && result.price > 0) {
        const entry = {
          ticker,
          code: result.futuresCode || ticker,
          price: result.price,
        ...
        results.push(entry);
        console.log(`✅ ${name} (${entry.code}): ${result.price} (${result.change >= 0 ? '+' : ''}${result.change}%)`);
      } else {
        console.warn(`⚠️ ${name}: 데이터 없음 (price=0)`);
        results.push({ ticker, code: '', price: 0, previousClose: 0, change: 0, failed: true });
      }
    } catch (err) {
      console.warn(`⚠️ ${name} 조회 실패: ${err.message}`);
      results.push({ ticker, code: '', price: 0, previousClose: 0, change: 0, failed: true });
    }
  }
