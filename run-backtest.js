/**
 * 백테스팅 실행 스크립트
 * 사용법: node run-backtest.js
 */

const backtest = require('./backend/backtest');

async function runBacktest() {
  console.log('🚀 투자 시스템 백테스팅 시작...\n');
  console.log('=' .repeat(80));

  try {
    // 백테스팅 실행 (30일 전부터, 7일 보유)
    const result = await backtest.runBacktest(30, 7);

    console.log('\n' + '='.repeat(80));
    console.log('📊 백테스팅 결과 요약');
    console.log('='.repeat(80));
    ...
    const fs = require('fs');
    fs.writeFileSync(
      './backtest-latest-results.json',
      JSON.stringify(result, null, 2)
    );
    console.log('💾 결과가 backtest-latest-results.json에 저장되었습니다.\n');
