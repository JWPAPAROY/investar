...
| **권장 매수일** | D+2 종가 |
| **권장 매도일** | D+9 종가 |
| **점수 모델 건강도** | ⛔ 역전 (ρ=-0.40, 밴드 4개) *(밴드 부족 — 참고용)* |
| **건강도 판정축** | 단조축(점수 높을수록 좋은가) |
| **TOP1 알파 (적용 중 timing)** | -2.15%p |
| **TOP1 알파 (권장 timing)** | -2.34%p |

## 진단 표본

- **권장 timing in-sample 평균**: -1.59%
- **권장 timing 최저주**: -21.06%
- **OOS 검증 수익**: N/A (n=0)
- **in-sample 기간**: 8주 / 표본 52건
- **평가 대상 추천 수**: 1470
...
- **baseline 대비 알파**: +0.78%p ⚪ baseline 동등

## ⚠️ 경고

- score_health: 밴드 4개짜리 상관 — 라벨은 참고용(주간 널뛰기 정상)
- 최신 in-sample 주(2026-08-17) 표본<3으로 권고 산출에서 제외 — 권고가 최근 레짐 미반영일 수 있음
- optimal timing quality=majority (현행 대비 우세 주 비율 83% (edge 평균 2.04%p, 최저주 -1.66%p)) — 연속 1주(<2)로 자동적용 보류, 권고만
---
