...
| 권장 timing | D+2 → D+9 |
| 그 주 적용 정책 | D+2 → D+10 |
| in-sample 평균 / 최저주 | -1.59% / -21.06% |
| OOS 검증 | N/A (n=0) |
| 점수 건강도 | inverted (ρ=-0.40, monotone) |
| TOP1 알파 (현재 / 권장) | -2.15%p / -2.34%p |
| 경고 | score_health: 밴드 4개짜리 상관 — 라벨은 참고용(주간 널뛰기 정상); 최신 in-sample 주(2026-08-17) 표본<3으로 권고 산출에서 제외 — 권고가 최근 레짐 미반영일 수 있음; optimal timing quality=majority (현행 대비 우세 주 비율 83% (edge 평균 2.04%p, 최저주 -1.66%p)) — 연속 1주(<2)로 자동적용 보류, 권고만 |
