...
**공시 이벤트 축 판정 — A1 무상증자만 생존**

DART 공시 549,260건(4.7년)을 수집해 사전 등록(`DISCLOSURE_VERDICT.md`)대로 판정했다.

- **1회차는 중단 조건에 걸려 무효.** 통제군(정기보고서)이 −0.69%p. 위약 테스트(무작위 5.4만 건)에서
  −0.66%가 나와 원인이 확정됐다 — 개별 종목을 분위 **평균**과 비교한 뒤 이벤트 사이에서 **중앙값**을
  취했다. 일간 수익률 우편향으로 하루 0.234%씩, 4일 누적 −0.9%p의 가짜 편향.
- **교정은 공시 데이터를 보지 않고** 위약 편향 ≤0.1% 기준만으로 골랐다(`disclosure-scale-calibration.js`).
  채택: **추정량 C** = 창 수익 − 같은 창을 온전히 관측한 동일일·동일 시총분위 동료의 중앙값.
  위약 실측 주 판정 구간 **+0.000%**. 임계값·유형·지평은 바꾸지 않았다.
- **2회차 판정**: ① A1_무상증자 +0.87%p ✅ / A2_자기주식취득 +0.54%p ✅ / A3 ❌
  ② A1 잔여비율 1.74 ✅ / A2 −10.22 ❌ · ③ 악재 청산 C군 전부 ❌ → **생존 = A1 하나.**
- 사용자 지적("공시보다 주가 반영이 빠르다")이 데이터로 확인됐다. 공시 전날까지 누적:
  무상증자 +5.03% · 공개매수 +4.14% · 최대주주변경 +2.50%, 통제군 0.00%.
  단 **A2는 사전 드리프트가 −0.76%로 없다** — 이사회 결의 즉시 공시라 유출 통로가 짧다는 가설과 부합.
- ⚠️ ② 명세 결함(사전 드리프트 ≤0이면 잔여비율이 무의미)을 발견했으나 **고치지 않았다.**
  성적을 본 뒤 공식을 고치면 골대 옮기기다. 다음 사전 등록에 면제 조항을 명시.

**자기주식취득 = 강제 흐름 축 — 기각**

`BUYBACK_VERDICT.md`로 별도 사전 등록 후 1회 판정(상세 1,988건).
- P0 척도 ✅(위약 +0.00% / 통제군 +0.01%p) · P4 반증 ✅(결과보고서 +0.04%p, n=870)
- P1 용량-반응 ✅ **Q1 +0.09 → Q5 +2.22, Spearman 1.00** (완전 단조, 시총 매칭 이후)
- P2 실행경로 ❌(직접 +0.78 vs 신탁 +1.24, 부호 반대) · P3 지속성 ❌(기간 종료 후 +0.38%p)
- 채택 조건 미충족 → **기각.** P2·P3 명세 결함(신탁 계약금액이 6~12개월 총액이라 규모가 다름 /
  롤링 프로그램) 을 사후 발견했으나 **수정하지 않았다.**

**🚨 market_flow_daily 종가에 수정주가가 덮여 있었다**

무상증자 실전 검증에서 일간 −40~−50%가 나왔다. 한국 주식은 일간 ±30% 제한이라 물리적으로 불가능하다.
- 원인: 수집이 최근 20일 창을 다시 채우는데, 권리락 이후 재수집된 행은 **수정주가**로 들어오고
  그 이전 행은 원주가로 남아 경계에 이음매가 생긴다.
  실측 대조 — 비비안 07-23: 로컬 KRX 7,080(−0.7%) vs `close` 3,544(**정확히 1/2**).
- 해결: `market_flow_daily.krx_close` 신설(KRX `TDD_CLSPRC`). `fetchKrxTradingValue` 가 이미
  파싱하고 있었으므로 배선만 했다. 소급 72거래일 / 184,144행.
- 검증: 세 종목의 급락일이 **신주배정기준일에서 계산한 권리락일로 정확히 이동**했다.
  수정주가 결함 80건 → 22건(−72%), '정상 반영' 식별 1 → 37건.
- 소비 측(`build-portfolio` / `bonus-issue-signals` / audit)을 `krx_close ?? close` 로.

**페이지네이션 행 유실 — 21곳**

`order()` 없이 `.range()` 로 페이징하면 페이지 간 순서가 보장되지 않는다.
실측(184,425행): **콜드 첫 호출에서 18,688행(10.1%) 중복+누락, 거래일 3일 증발.**
2·3회차는 멀쩡했다 — 하루 한 번 도는 cron 이 정확히 그 조건이라 운영에서만 재현되는 버그였다.
`backend/supabasePaging.js` 신설(PK 단일 출처, 미등록 테이블은 throw) 후 21곳 적용.
단 641/1,133일 대형 판정 스크립트는 로컬 KRX 파일을 읽으므로 **기존 결론은 영향 없다.**

**무상증자 실전 파이프라인**

`bonus_issue_signals` + `scripts/bonus-issue-signals.js` + `bonus-signals.yml` + 프론트 📢 탭.
- 권리락을 **건별로** 막는다(`fricDecsn` 신주배정기준일 → 직전 거래일). 실측 권리락은 중앙 D+10 —
  매도일(D+5) 뒤 5거래일뿐이라 평균으로는 못 막는다. 상세 확보율 99.2%.
- 저녁 배치가 **다음 거래일 지시**를 보내므로 GitHub Actions 지연(중앙 4시간)이 무해하다.
- 원수익과 **매칭초과**를 둘 다 기록한다. 첫 유효 비교: 매칭초과 중앙 −2.00%(n=10) vs
  백테스트 +1.25%(n=248). **n=10으로 판단하지 않는다.**

**정리**

- `api/health.js` 제거 — 2025-10-24 스캐폴딩 이후 11개월간 호출처 0, DB 연결도 확인하지 않았다.
- 선행 지표 전면 제거 — `volumeDnaExtractor.js` · `api/patterns/volume-dna.js` · 프론트 🧬 DNA 탭.
  To-Do #6-B 의 재개 조건(#6-A 알파)이 깔때기 기각 확정으로 **영구 불성립**. 실사용 0회.
- 저변동성 실시간 관측 종료 — 판정이 2.5년 표본으로 이관됐는데 관측만 매일 돌고 있었다.
- 일회성 SQL 5개 삭제. `supabase-schema-full.sql` 재덤프(테이블 14→18).
- **⚠️ Vercel 서버리스 함수 12개 한도** — 12를 채운 배포가 실패했다(GitHub Actions 는 초록불이라
  눈치채기 어렵다). 현재 9개. 새 엔드포인트는 쿼리로 얹거나 `_접두` 모듈로.
- 탭 8 → 7, `index.html` 6,622 → 6,148줄.

### v3.97 (2026-08-26)

**풀 재설계** — "문제는 랭킹이 아니라 풀"(2026-08-08)이라는 결론의 실행. 현행 추천 경로와 완전히 분리된 별도 라인으로 만들고 프론트엔드에 나란히 띄운다.

- **현행 풀의 구조적 모순 측정**: 추천 풀은 KIS 순위 API 5종(거래량 증가율·거래량·거래대금·회전율·등락률)으로 만들어진다 — 다섯 축이 전부 "이미 움직인/이미 거래가 몰린" 주목도다. 실측: **풀의 65%가 시총 3천억 미만인데 최종 TOP3의 80%는 시총상위300**(1,010건 기준). 마이크로캡을 잔뜩 모은 뒤 마지막 시총 플로어로 내다 버리는 구조이고, TOP3가 49일 중 28일 미달인 이유도 이것이다. → **시총 기준은 마지막 필터가 아니라 유니버스여야 한다.**
- **저PBR·저변동 포트폴리오 신설** (`backend/portfolio.js` · `scripts/build-portfolio.js` · `api/portfolio` · 프론트 📦 탭): 유니버스 = 보통주 · 시총 3,000억+ · 20일 평균 거래대금 10억+ 중 시총 상위 300. 팩터 = 저PBR 백분위 + 저변동(20일 σ) 백분위. K=10 / 20거래일 / 시총가중.
  - **리밸런싱 주기를 60일→20일로 바꿨다.** 60일의 우위가 표본 착시였다: 검증 CAGR 20일 +65.5%(24기간) vs 60일 +45.9%(7기간)이고, **같은 기간 시총가중 시장과 비교하면 20일은 +7.4%p 우위, 60일은 −11.1%p 열위**다. 60일을 골랐던 근거(IS 승률 78%·MDD −6.5%)는 9기간 중 7회짜리 숫자였다. 바꾼 이유는 수익이 아니라 표본 3배 + 벤치마크 우위. 치르는 값은 MDD −5.7%→−10.2%.
  - 수급·모멘텀·거래량·업종·손절은 개별 검정을 통과하지 못해 **일부러 넣지 않았다**(화면에도 명시).
  - 재무 point-in-time 규칙(분기말+45일/연말+90일)을 공용 모듈로 승격 — 실운용과 백테스트가 다른 지연을 쓰면 성과가 비교 불가능해진다.
- **판정 기준 사전 등록** (`PORTFOLIO_VERDICT.md` + `scripts/portfolio-verdict.js`): 판정일 2027-02-26, ① 초과수익 ≥0%p 통과/<−5%p 기각 ② MDD −20% 이내 ③ 현행 TOP3 대비는 참고만. **애매(0~−5%p)는 연장 없이 기각으로 처리한다.** 스크립트가 판정일 전에는 값만 보여주고 판정을 거부하며, 파라미터가 바뀌면 연속성 단절을 경고한다. 저변동성 62일 착시·60일 9기간 승률·깔때기 3회 기각이 전부 사후 기준 선택의 결과였다.

**출처 대조** — 지적으로 시작해 세 건이 연달아 드러났다.

- **🚨 "거래대금은 KIS에 없다"는 v3.96의 판단이 틀렸다**: TR 하나(FHKST01010400)의 한계를 벤더 전체로 일반화한 것이었다. `FHKST03010100`(국내주식기간별시세)은 `acml_tr_pbmn`을 준다. **실측 대조 2,382종목 전부 일치(0.000%)**. v3.96이 만든 KRX 우회(161,516행 백필 + 매 수집 2콜 추가 + 당일 하루 지연)는 전부 불필요했다.
  - 같은 TR 교체로 두 가지가 더 해결됐다: 기존 TR은 **30일이 상한**이라 days=100을 요청하던 백필 스크립트 2개가 조용히 30개만 받고 있었다(→100행 확인). 그리고 `prtt_rate`·`mod_yn`·`flng_cls_code`가 붙어 와 액면분할로 인한 종가 불일치를 판별할 수 있다.
  - 부수: 조회 종료일이 UTC라 00~09시 KST에 전날을 조회하고 있었다 → KST 정정.
- **🚨 `market_cap`이 KIS 역산 근사치였다**: `shares = 현재시총/현재가` → 과거 종가에 곱하는 방식. 실측 대조에서 **NAVER −3.1%, 018700 +42.7%, 최대 30%(알테오젠 21.3조 vs 16.4조)**. 3% 초과가 하루 8~17종목, cap300 안에도 2종목. 포트폴리오 유니버스가 이 값으로 정해지고 **백테스트는 KRX MKTCAP으로 검증됐으므로** 실운용이 다른 시총을 쓰면 검증 성적이 적용되지 않는다.
  - `krx_market_cap`·`krx_listed_shares` 컬럼 추가(덮어쓰지 않고 나란히 — 덮어쓰면 언제부터 어긋났는지 되짚을 수 없다). KRX는 다음 날 공표라 **당일은 최신 상장주식수 × 당일 종가**로 계산한다(NAVER 8/25에서 실측과 정확히 일치 확인). 신호일 출처를 로그로 찍고 KIS 폴백이 과반이면 경고.
  - 커버리지도 KRX가 낫다 — 시총 null이 KIS 217종목 vs KRX 32종목.
  - 시총을 바꾸자 포트폴리오 10종목 중 1개가 교체됐다(CJ제일제당 → 롯데케미칼).
- **`scripts/compare-sources.js` 신설 + 매일 자동 기록**: 종가·거래량·거래대금·시총(KIS역산/KRX저장)을 대조해 `source_reconciliation`에 저장하고 운영 진단 탭에 노출한다. **수급(투자자별)은 KRX OpenAPI에 경로 자체가 없어(404, 승인 문제 아님) 대조 불가** — 실행할 때마다 그 사실을 출력한다.
  - 대조 결과(2026-08-25, 2,551종목): 거래량 100.00% / 거래대금 100.00% / 시총(KRX저장) 100.00% / 종가 99.92%(액면분할 2종목) / 시총(KIS역산) 99.79%.
- **`.github/workflows/portfolio.yml`**: collect-market-flow 완료 후 리밸런싱 + 대조를 자동 실행. 주기 판단은 스크립트가 하므로 매일 돌아도 멱등. 대조는 `if: always()`로 분리해 리밸런싱이 실패해도 기록은 남긴다.
- **`server.js`: express.static 을 API 라우트 뒤로 이동** — 정적 서빙이 요청 경로와 같은 이름의 디렉터리(`api/portfolio`, `api/stocks`)를 만나면 301로 리다이렉트해 `/api/portfolio`가 HTML을 반환하고 있었다(로컬 개발 한정).
