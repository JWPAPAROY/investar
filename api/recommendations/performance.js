...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // v3.71: 모멘텀 분석 모드 — ?momentum=true
    if (req.query.momentum === 'true') {
...
async function handleMomentum(req, res, apiStartTime) {
  ...
  const kstMinutes = kstHour * 60 + kstNow.getMinutes();
  let trackTime;
